...
export default function StudentDashboardPage() {
  ...
  function navigateToSection(sectionKey, sectionId) {
    ...
    <div className="dashboard-shell">
          ...
          <nav className="admin-sidebar-nav" aria-label="Student drawer links">
            <a
              ...
              onClick={(event) => {
                ...
                navigateToSection("overview", "student-overview");
              }}
            >
            ...
            <a
              className={`admin-sidebar-link ${activeSection === "faculty" ? "active" : ""}`}
              href="#student-faculty-modules"
              onClick={(event) => {
                event.preventDefault();
                navigateToSection("faculty", "student-faculty-modules");
              }}
            >
              <span className="admin-sidebar-link-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" focusable="false">
                  ...
                  <path d="M7 10.5V15" />
                  <path d="M17 10.5V15" />
                  <path d="M4.5 13.5c2 2 5 3.5 7.5 3.5s5.5-1.5 7.5-3.5" />
                </svg>
              </span>
              <span>Modules</span>
            </a>
            <a
              ...
              href="#student-browse-quizzes"
              onClick={(event) => {
                event.preventDefault();
                navigateToSection("browse", "student-browse-quizzes");
              }}
            >
              <span className="admin-sidebar-link-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" focusable="false">
            ...
            <a
              ...
              onClick={(event) => {
                event.preventDefault();
                navigateToSection("results", "student-my-results");
              }}
            >
              <span className="admin-sidebar-link-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" focusable="false">
            ...
            <Link className="admin-sidebar-link" to="/student/settings" onClick={() => setIsMobileMenuOpen(false)}>
              <span className="admin-sidebar-link-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" focusable="false">
      ...
      <main className="dashboard-main student-layout student-layout-dashboard">
        ...
        <section className="student-content">
          <section className="dashboard-grid student-stats-grid" id="student-overview" aria-label="Student overview cards">
          ...
          <section className="quiz-section student-results-section" id="student-my-results" aria-label="My quiz results">
            <div className="submission-head">
              <div>
                <h2>My Submissions</h2>
                <p>Your quiz history and personal results.</p>
              </div>
              ...
              <table className="submission-table student-results-table">
                ...
                <tbody>
                  {filteredResults.map((result) => {
                    ...
                    return [
                      <tr
                        key={`${result.id || `${result.quizTitle}-${result.submittedAt}`}-row`}
                        className={`student-result-row${isHighScore ? " is-high-score" : ""}`}
                      >
                        ...
                        <td data-label="Status">
                          <span className={`quiz-result-status-badge ${status === "Pass" ? "is-pass" : "is-fail"}`}>
                            {status}
                          </span>
                        </td>
                      ...
                      isOpen ? (
                        <tr
                          key={`${result.id || `${result.quizTitle}-${result.submittedAt}`}-detail`}
                          className="student-result-detail-row"
                        >
          ...
          <section className="quiz-section" id="student-faculty-modules" aria-label="Faculty modules">
            <div className="quiz-section-header">
              <h2>Faculty Modules</h2>
              <span className="quiz-count-badge">
                {categories.length > 1 ? categories.length - 1 : 0} module
                {categories.length - 1 === 1 ? "" : "s"}
              </span>
            </div>
            <div className="module-grid">
              {(categories.length > 1 ? categories.slice(1) : ["General"]).map((moduleName) => (
                <article key={moduleName} className="module-card">
                  <h3>{moduleName}</h3>
                  <p>Module available for your current faculty track.</p>
                </article>
              ))}
            </div>
          </section>

          <section className="quiz-section" id="student-browse-quizzes" aria-label="Browse available quizzes">
            <div className="quiz-section-header">
      ...
      <nav className="student-bottom-nav" aria-label="Student quick navigation">
        ...
        <a
          href="#student-faculty-modules"
          className={`student-bottom-nav-link ${activeSection === "faculty" ? "active" : ""}`}
          onClick={(event) => {
            event.preventDefault();
            navigateToSection("faculty", "student-faculty-modules");
          }}
